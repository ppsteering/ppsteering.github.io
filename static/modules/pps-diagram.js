...
(function () {
  ...
  // VARIANT 0 — COMBINED: flow-field funnel + steered distribution + one bold rollout
  Diagram.prototype.drawCombined = function (ctx, p, ts) {
    this.scaffold(ctx, p, {});
    var g = this.gamma, L = meanAttractor(g), self = this;

    // (1) faint steered velocity field, funnelling toward L(γ)
    var cols = 13, rows = 9;
    for (var ci = 0; ci < cols; ci++) for (var ri = 0; ri < rows; ri++) {
      var wx = lerp(WORLD.x0 + 0.12, WORLD.x1 - 0.12, ci / (cols - 1));
      var wy = lerp(WORLD.y0 + 0.10, WORLD.y1 - 0.10, ri / (rows - 1));
      var dx = L.x - wx, dy = L.y - wy, m = Math.hypot(dx, dy) + 1e-6;
      var len = Math.min(0.085, m * 0.5);
      var a = this.W2S({ x: wx, y: wy }), b = this.W2S({ x: wx + dx / m * len, y: wy + dy / m * len });
      var prox = Math.max(0, 1 - m / 1.4);
      ctx.globalAlpha = 0.10 + prox * 0.30;
      arrow(ctx, a, b, p.sub, 1.2, 4);
    }
    ctx.globalAlpha = 1;

    // (2) steered distribution — clean cloud of sampled steered actions (dots only)
    ctx.globalAlpha = 0.82;
    for (var dc = 0; dc < this.dist.length; dc++) {
      var dsc = this.dist[dc], spc = this.W2S({ x: lerp(dsc.bi.x, dsc.ti.x, g), y: lerp(dsc.bi.y, dsc.ti.y, g) });
      ctx.beginPath(); ctx.arc(spc.x, spc.y, 2.3, 0, Math.PI * 2); ctx.fillStyle = p.pps; ctx.fill();
    }
    ctx.globalAlpha = 1;

    // pick the bold single rollout: the sample landing nearest the mean target L
    var boldIdx = 0, best = 1e9;
    for (var i = 0; i < this.particles.length; i++) {
      var pt = this.particles[i];
      var Lx = lerp(pt.bi.x, pt.ti.x, g), Ly = lerp(pt.bi.y, pt.ti.y, g);
      var dd = Math.hypot(Lx - L.x, Ly - L.y);
      if (dd < best) { best = dd; boldIdx = i; }
    }

    // (3) a few animated faint rollouts (sampling in motion)
    for (var k = 0; k < this.particles.length; k += 2) {
      if (k === boldIdx) continue;
      var ptk = this.particles[k];
      var sk = Math.max(0, Math.min(1, this.phase + ptk.off));
      var pathk = this.integrate(ptk, g, sk);
      ctx.beginPath();
      for (var j = 0; j < pathk.length; j++) { var spk = this.W2S(pathk[j]); j ? ctx.lineTo(spk.x, spk.y) : ctx.moveTo(spk.x, spk.y); }
      ctx.strokeStyle = rgbaStr(p.sub, 0.16); ctx.lineWidth = 1.0; ctx.lineCap = 'round'; ctx.stroke();
    }

    // (4) the single bold rollout -> steered target, with the velocity-vector inset
    var lead = this.particles[boldIdx];
    ...
    this.legendCard(ctx, p, [
      ...
      { type: 'dots', color: p.pps, label: 'steered action distribution' },
      { type: 'quiver', color: p.sub, label: 'v_PPS field' }
    ]);
  };
  ...
  window.PPSDiagram = Diagram;
  window.PPS_successAt = successAt;
})();
